...
CORS(
    ...
    resources={r"/api/*": {
        "origins": "https://ai-difference-checker-fr-git-c3640a-pradeepacharya123s-projects.vercel.app"
    }}
)
# -----------------------
...
def upload():
    ...
    Returns:
      - html_diff: HTML table
      - total_changes, add_pct, del_pct
      - diff_text: plain text diff (used for summary)
    """
    ...
    # Return JSON
    return jsonify({
        "html_diff": results["html_diff"],  # HTML diff table
        "total_changes": results["total"],
        "add_pct": results["add_pct"],
        "del_pct": results["del_pct"],
        "diff_text": results["diff_text"]   # raw diff lines
    }), 200
...
def summary():
    ...
    # Limit characters to avoid very large payloads
    MAX_CHARS = 30000
